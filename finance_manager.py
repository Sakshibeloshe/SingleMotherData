...
class SingleMotherFinanceManager:
    ...
    def get_recommendations(self):
        ...
        return budget, jobs, emergency_fund
...
def calculate_finance():
    try:
        ...
        budget, jobs, emergency_fund = finance_manager.get_recommendations()

        budget_text = "\n".join([f"{k}: Rs.{v:.2f}" for k, v in budget.items()])
        jobs_text = "\n".join(jobs)
        result_label.config(text=f"Budget Allocation:\n{budget_text}\n\nJob Recommendations:\n{jobs_text}\n\nEmergency Fund: Rs.{emergency_fund:.2f}")
